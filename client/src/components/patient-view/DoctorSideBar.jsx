...
const DoctorSelector = ({ selectedId, setSelectedId }) => {
  ...
  return (
    <main className="flex items-center justify-center min-h-screen p-4 sm:p-6 bg-[#0c0a1a] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops)),_radial-gradient(ellipse_at_bottom,_var(--tw-gradient-stops))] from-indigo-900 via-transparent to-transparent animate-aurora font-['Inter',_sans-serif]">
      <div className="w-full max-w-md p-6 text-center text-white bg-slate-900/40 border border-white/10 rounded-3xl shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-3">
          ...
          {ALL_SPECIALIZATIONS.map((specialty) => (
            <button
              ...
              className={`${cardBaseClasses} ${cardHoverClasses} ${
                selectedId === specialty.id ? cardSelectedClasses : ""
              }`}
            >
              <div
              ...
              >
                {specialtyIcons[specialty.id] || <FaStethoscope />}{" "}
                {/* Fallback icon */}
              </div>
              <span
                className={`font-medium text-sm transition-all duration-300 ${
                  selectedId === specialty.id ? "font-bold" : "text-white/90"
                }`}
              >
                {specialty.label}
              </span>
        ...
        <footer className="flex items-center justify-center min-h-[50px] p-4 mt-10 text-lg bg-black/20 rounded-xl">
          {selectedLabel ? (
            ...
            <p className="text-white/60">Please select a specialty</p>
          )}
        </footer>
      </div>
    </main>
